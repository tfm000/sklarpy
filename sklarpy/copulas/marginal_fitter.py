# Contains code for the MarginalFitter class, which fits univariate distributions to each random variable in a multivariate sample
import numpy as np
import pandas as pd
...
class MarginalFitter(Savable):
    """Class used to fit multiple distributions to data easily and to calculate cdf/distribution values to use with copulas."""
    _OBJ_NAME = 'MarginalFitter'

    def __init__(self, data: Union[pd.DataFrame, np.ndarray], name: str = None):
        """Used for fitting univariate distributions to each random variable in a given multivariate dataset.
    ...
    def _fit_check(self):
        """raises an error if MarginalFitter object has not been fitted to data."""
        if not self._fitted:
            raise FitError('MarginalFitter object has not been fitted.')

    def _pdfs_cdf_ppfs_logpdfs_inputs(self, func_str: str, x: Union[pd.DataFrame, np.ndarray], match_datatype: bool = True) -> Union[pd.DataFrame, np.ndarray]:
        """Utility function able to implement pdf, cdf, ppf and logpdf methods without duplicate code.
    ...
    def marginal_pdfs(self, x: Union[pd.DataFrame, np.ndarray] = None, match_datatype: bool = True) -> Union[pd.DataFrame, np.ndarray]:
    ...
    def marginal_cdfs(self, x: Union[pd.DataFrame, np.ndarray] = None, match_datatype: bool = True) -> Union[pd.DataFrame, np.ndarray] :
    ...
    def marginal_ppfs(self, q: Union[pd.DataFrame, np.ndarray],  match_datatype: bool = True) -> Union[pd.DataFrame, np.ndarray]:
    ...
    def marginal_logpdfs(self, x: Union[pd.DataFrame, np.ndarray] = None, match_datatype: bool = True) -> Union[pd.DataFrame, np.ndarray] :
        """Calculates the pdf values for each univariate marginal distribution for a given set of observations x.
    ...
    def marginal_rvs(self, size: int, match_datatype: bool = True) -> Union[pd.DataFrame, np.ndarray]:
    ...
    def pairplot(self, color: str = 'royalblue', alpha: float = 1.0, figsize: tuple = (8, 8), grid: bool = True,
                 axes_names: Iterable = None, plot_kde: bool = True, num_generate: int = 10 ** 3, show: bool = True) -> None:
        ...
        if axes_names is None:
            pass
        elif not (isinstance(axes_names, Iterable) and len(axes_names) == self._num_variables):
    ...
    def fitted(self) -> bool:
        """True if the MarginalFitter object has been fitted to data. False otherwise."""
        return self._fitted
