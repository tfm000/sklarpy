...
class PreFitCopula(NotImplementedBase):
    ...
    def __init__(self, name: str, mv_object: PreFitContinuousMultivariate):
        ...
        name : str
            ...
            Used when saving, if a file path is not specified and/or for additional identification purposes.
        mv_object : PreFitContinuousMultivariate
            The multivariate distribution which defines the copula distribution.
        """
        self._name: str = name
    ...
    def _get_data_array(self, data: Union[pd.DataFrame, np.ndarray], is_u: bool) -> np.ndarray:
        """Converts the user's data input into a numpy array and performs checks.
        ...
        data : Union[pd.DataFrame, np.ndarray]
            ...
            Can be observations of random variables or marginal cdf / pseudo-observation values.
        is_u : bool
            True if the user data is marginal cdf / pseudo-observation values. False otherwise.
    ...
    def _get_mdists(self, mdists: Union[MarginalFitter, dict], d: int, check: bool = True) -> Dict[int, FittedUnivariateBase]:
        """Converts the user's marginal distributions into a standardized dictionary
        format and performs checks.

        Parameters
        ----------
        mdists : Union[MarginalFitter, dict]
            The fitted marginal distributions of each random variable.
            Must be a fitted MarginalFitter object or a dictionary with numbered indices as values and
            fitted SklarPy univariate distributions as values. The dictionary indices must correspond
    ...
    def __mdist_calcs(self, func_strs: List[str], data: np.ndarray, mdists: Union[MarginalFitter, dict], check: bool, funcs_kwargs: dict = None) -> Dict[str, np.ndarray]:
        """Utility function able to evaluate functions of the univariate marginal distributions.

        Parameters
        ----------
        func_strs : List[str]
            List containing names of univariate distribution functions to implement.
        data: np.ndarray
        ...
        res: Dict[str, np.ndarray]
            A dictionary with func_strs as keys and numpy arrays of outputs as values.
        """
        ...
        for func_str in func_strs:
            ...
            if func_str not in dir(FittedUnivariateBase):
                raise NotImplementedError(f"{func_str} not implemented in FittedUnivariateBase.")

            # evaluating the function for each variable and its respective marginal dist
            ...
            for index, dist in mdists_dict.items():
                ...
                vals: np.ndarray = eval(f"dist.{func_str}(data_i, **func_kwargs)")
    ...
    def logpdf(self, x: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], match_datatype: bool = True, **kwargs) -> Union[pd.DataFrame, np.ndarray]:
        ...
        x: Union[pd.DataFrame, np.ndarray]
            The values to evaluate the log-pdf function of the joint distribution at.
        ...
        mdists : Union[MarginalFitter, dict]
            ...
            to the indices of the variables.
        match_datatype: bool
            True to output the same datatype as the input. False to output a np.ndarray.
            Default is True.
        kwargs:
        ...
        res: dict = self.__mdist_calcs(func_strs=['cdf', 'logpdf'], data=x_array, mdists=mdists_dict, check=True)
    ...
    def pdf(self, x: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], match_datatype: bool = True, **kwargs) -> Union[pd.DataFrame, np.ndarray]:
        ...
        x: Union[pd.DataFrame, np.ndarray]
            The values to evaluate the pdf function of the joint distribution at.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
            Can be a Params object or the specific multivariate distribution or a tuple containing
            these parameters in the correct order.
    ...
    def __cdf_mccdf(self, mc_cdf: bool, x: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], match_datatype: bool = True, **kwargs) -> Union[pd.DataFrame, np.ndarray]:
        """Utility function able to implement cdf and mc_cdf methods without duplicate code.
        ...
        x: Union[pd.DataFrame, np.ndarray]
            The values to evaluate the cdf / mc_cdf function of the joint distribution at.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
            Can be a Params object or the specific multivariate distribution or a tuple containing
            these parameters in the correct order.
        ...
        """
        x_array: np.ndarray = self._get_data_array(data=x, is_u=False)
        res: dict = self.__mdist_calcs(func_strs=['cdf'], data=x_array, mdists=mdists, check=True)
    ...
    def cdf(self, x: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict],  match_datatype: bool = True, **kwargs) -> Union[pd.DataFrame, np.ndarray]:
        ...
        This may take time to evaluate for certain copula distributions, due to d-dimensional numerical integration.
        In these case, mc_cdf will likely evaluate faster.

        x: Union[pd.DataFrame, np.ndarray]
            The values to evaluate the cdf function of the joint distribution at.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
            Can be a Params object or the specific multivariate distribution or a tuple containing
            these parameters in the correct order.
    ...
    def mc_cdf(self, x: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], match_datatype: bool = True, num_generate: int = 10 ** 4, show_progress: bool = False, **kwargs) -> Union[pd.DataFrame, np.ndarray]:
        """The monte-carlo numerical approximation of the cdf function of the overall joint distribution.
        The standard cdf function may take time to evaluate for certain copula distributions, due to d-dimensional numerical integration.
        ...
        match_datatype: bool
            ...
            Default is True.
        num_generate: int
            The number of random numbers to generate to use when numerically approximating the joint cdf using monte-carlo.
        show_progress: bool
        ...
        """
        return self.__cdf_mccdf(mc_cdf=True, x=x, copula_params=copula_params, mdists=mdists, match_datatype=match_datatype, num_generate=num_generate, show_progress=show_progress, **kwargs)

    def rvs(self, size: int, copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], ppf_approx: bool = True) -> np.ndarray:
        """The random variable generator function of the overall joint distribution.
        This requires the evaluation of the ppf / quantile function of each marginal distribution,
        which for certain univariate distributions requires the evaluation of an integral and may be time-consuming.
        The user therefore has the option to use the ppf_approx / quantile approximation function in place of this.

        Parameters
        ----------
        size: int
            How many multivariate random samples to generate from the overall joint distribution.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
            Can be a Params object or the specific multivariate distribution or a tuple containing
            these parameters in the correct order.
        mdists : Union[MarginalFitter, dict]
            ...
            to the indices of the variables.
        ppf_approx: bool
            True to use the ppf_approx function to approximate the ppf / quantile function,
            via linear interpolation, when generating random variables.
            Default is True.

        Returns
        -------
        rvs: np.ndarray
            Multivariate array of random variables, sampled from the joint distribution.
        """
        ...
        res: dict = self.__mdist_calcs(func_strs=[func_str], data=copula_rvs, mdists=mdists, check=True)
        return res[func_str]
    ...
    def copula_logpdf(self, u: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], match_datatype: bool = True, **kwargs) -> Union[pd.DataFrame, np.ndarray]:
        ...
        u: Union[pd.DataFrame, np.ndarray]
            The cdf / pseudo-observation values to evaluate the log-pdf function of the copula distribution at.
            Each ui must be in the range (0, 1) and should be the cdf values of the univariate marginal distribution
    ...
    def copula_pdf(self, u: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], match_datatype: bool = True, **kwargs) -> Union[pd.DataFrame, np.ndarray]:
        ...
        u: Union[pd.DataFrame, np.ndarray]
            The cdf / pseudo-observation values to evaluate the pdf function of the copula distribution at.
            Each ui must be in the range (0, 1) and should be the cdf values of the univariate marginal distribution
            of the random variable xi.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
    ...
    def __copula_cdf_mccdf(self, mc_cdf: bool, u: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], match_datatype: bool = True, **kwargs) -> Union[pd.DataFrame, np.ndarray]:
        """Utility function able to implement copula_cdf and copula_mc_cdf methods without duplicate code.
        ...
        x: Union[pd.DataFrame, np.ndarray]
            The values to evaluate the copula_cdf / copula_mc_cdf function of the joint distribution at.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
            Can be a Params object or the specific multivariate distribution or a tuple containing
            these parameters in the correct order.
    ...
    def copula_cdf(self, u: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], match_datatype: bool = True, **kwargs) -> Union[pd.DataFrame, np.ndarray]:
        ...
        This may take time to evaluate for certain copula distributions, due to d-dimensional numerical integration.
        In these case, copula_mc_cdf will likely evaluate faster.

        u: Union[pd.DataFrame, np.ndarray]
            The cdf / pseudo-observation values to evaluate the cdf function of the copula distribution at.
            Each ui must be in the range (0, 1) and should be the cdf values of the univariate marginal distribution
            of the random variable xi.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
    ...
    def copula_mc_cdf(self, u: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], match_datatype: bool = True, num_generate: int = 10 ** 4, show_progress: bool = False, **kwargs) -> Union[pd.DataFrame, np.ndarray]:
        """The monte-carlo numerical approximation of the cdf function of the copula distribution.
        The standard copula_cdf function may take time to evaluate for certain copula distributions, due to d-dimensional numerical integration.
        In these cases, copula_mc_cdf will likely evaluate faster
        ...
        match_datatype: bool
            ...
            Default is True.
        num_generate: int
            The number of random numbers to generate to use when numerically approximating the copula cdf using monte-carlo.
        show_progress: bool
        ...
        """
        return self.__copula_cdf_mccdf(mc_cdf=True, u=u, copula_params=copula_params, match_datatype=match_datatype, num_generate=num_generate, show_progress=show_progress, **kwargs)
    ...
    def copula_rvs(self, size: int, copula_params: Union[Params, tuple]) -> np.ndarray:
        ...
        ----------
        size: int
            How many multivariate random samples to generate from the copula distribution.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
            Can be a Params object or the specific multivariate distribution or a tuple containing
            these parameters in the correct order.
        ...
        -------
        rvs: np.ndarray
            Multivariate array of random variables, sampled from the copula distribution.
            These correspond to randomly sampled cdf / pseudo-observation values of the univariate marginals.
        """
    ...
    def _get_components_summary(self, fitted_mv_object: FittedContinuousMultivariate, mdists: dict, typekeeper: TypeKeeper) -> pd.DataFrame:
        """Creates a summary of the marginal distributions and the fitted multivariate copula distribution.
        ...
        mdists : dict
            The fitted marginal distributions of each random variable.
            Must a dictionary with numbered indices as values and fitted SklarPy univariate distributions as values.
            The dictionary indices must correspond to the indices of the variables.
        ...
        component_summary: pd.DataFrame
            A summary of the marginal distributions and the fitted multivariate copula distribution.
        """
    ...
    def num_marginal_params(self, mdists: Union[MarginalFitter, dict]) -> int:
        """Calculates the total number of parameters defining the marginal distributions.
    ...
    def num_copula_params(self, copula_params: Union[Params, dict]) -> int:
        """Calculates the number of parameters defining the copula distribution.
    ...
    def num_scalar_params(self, mdists: Union[MarginalFitter, dict]) -> int:
        """Calculates the number of scalar parameters defining the overall joint distribution.
        ...
        num_scalar_params: int
            The number of scalar parameters defining the overall joint distribution.
        """
    ...
    def num_params(self, mdists: Union[MarginalFitter, dict]) -> int:
        """Calculates the number of parameters defining the overall joint distribution.
    ...
    def fit(self, data: Union[pd.DataFrame, np.ndarray, None] = None, copula_params: Union[Params, tuple, None] = None, mdists: Union[MarginalFitter, dict, None] = None, **kwargs) -> FittedCopula:
        """Fits the overall joint distribution to a given dataset or user provided parameters.
        ...
        copula_params: Union[Params, tuple, None]
            ...
            Can be a Params object or the specific multivariate distribution or a tuple containing
            these parameters in the correct order.
            If not passed, user must provide a dataset to fit too.
        mdists : Union[MarginalFitter, dict, None]
            ...
            If not passed, user must provide a dataset to fit too.
        kwargs:
            kwargs to pass to MarginalFitter.fit and / or the relevant multivariate distribution's .fit method
            See below

        Keyword Arguments
        -----------------
        univariate_fitter_options: dict
            User provided arguments to use when fitting each marginal distribution.
            See MarginalFitter.fit documentation for more.
        show_progress: bool
        ...
        if copula_params is None:
            ...
            res: dict = self.__mdist_calcs(func_strs=['cdf'], data=data_array, mdists=mdists_dict, check=False)
    ...
    def likelihood(self, data: Union[np.ndarray, pd.DataFrame], copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict]) -> float:
        ...
        data : Union[np.ndarray, pd.DataFrame]
            The values to evaluate the likelihood function of the joint distribution at.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
            Can be a Params object or the specific multivariate distribution or a tuple containing
            these parameters in the correct order.
    ...
    def loglikelihood(self, data: Union[np.ndarray, pd.DataFrame], copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict]) -> float:
        ...
        ----------
        data : Union[np.ndarray, pd.DataFrame]
            The values to evaluate the loglikelihood function of the joint distribution at.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
            Can be a Params object or the specific multivariate distribution or a tuple containing
            these parameters in the correct order.
    ...
    def aic(self, data: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict]) -> float:
        """The Akaike Information Criterion (AIC) function of the overall joint distribution.

        Parameters
        ----------
        data : Union[np.ndarray, pd.DataFrame]
            The values to evaluate the AIC function of the joint distribution at.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
            Can be a Params object or the specific multivariate distribution or a tuple containing
            these parameters in the correct order.
    ...
    def bic(self, data: Union[pd.DataFrame, np.ndarray], copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict]) -> float:
        """The Bayesian Information Criterion (BIC) function of the overall joint distribution.

        Parameters
        ----------
        data : Union[np.ndarray, pd.DataFrame]
            The values to evaluate the BIC function of the joint distribution at.
        copula_params: Union[Params, tuple]
            The parameters of the multivariate distribution used to specify your copula distribution.
            Can be a Params object or the specific multivariate distribution or a tuple containing
            these parameters in the correct order.
    ...
    def marginal_pairplot(self, copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], ppf_approx: bool = True, color: str = 'royalblue', alpha: float = 1.0, figsize: tuple = (8, 8),
                          grid: bool = True, axes_names: tuple = None, plot_kde: bool = True, num_generate: int = 10 ** 3, show: bool = True) -> None:
        """Produces a pair-plot of each fitted marginal distribution.

        This requires the sampling of multivariate random variables from the joint copula distribution,
        which requires the evaluation of the ppf / quantile function of each marginal distribution,
        which for certain univariate distributions requires the evaluation of an integral and may be time-consuming.
        The user therefore has the option to use the ppf_approx / quantile approximation function in place of this.
        ...
        axes_names: Iterable
            ...
            If provided, must be an iterable with the same length as the number of variables.
            If None provided, the axes will be labeled using the names of the variables provided
            during the joint distribution fit, if possible.
        plot_kde: bool
            True to plot the KDE of your marginal distributions in the diagonal plots
            Default is True.
        num_generate: int
            The number of random variables to generate from each marginal distribution, to use as data for the pair-plots.
            Default is 1000.
        show: bool
    ...
    def _threeD_plot(self, func_str: str, copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], ppf_approx: bool, var1_range: np.ndarray, var2_range: np.ndarray, color: str, alpha: float,
                 figsize: tuple, grid: bool, axes_names: Iterable, zlim: tuple,
                 num_generate: int, num_points: int, show_progress: bool, show: bool, mc_num_generate: int = None, ranges_to_u: bool = False) -> None:
        """Utility function able to implement pdf_plot, cdf_plot, mc_cdf_plot, copula_pdf_plot, copula_cdf_plot and copula_mc_cdf_plot
        methods without duplicate code.

        Note that these plots are only implemented when we have 2-dimensional / bivariate distributions.

        This may require the sampling of multivariate random variables from the joint copula distribution,
        which requires the evaluation of the ppf / quantile function of each marginal distribution,
        which for certain univariate distributions requires the evaluation of an integral and may be time-consuming.
        The user therefore has the option to use the ppf_approx / quantile approximation function in place of this.
        ...
        var1_range: np.ndarray
            ...
            If None passed, then an evenly spaced array of length num_points will be generated,
            whose upper and lower bounds are (0.01, 0.99) for copula function plots. For joint distribution
            plots, the upper and lower bounds are determined by generating a multivariate random sample of
            size num_generate and then taking the observed min and max values of each variable from this sample.
        ...
        num_generate: int
            The number of random variables to generate from the joint distribution, to determine the bounds of
            our variables, if necessary.
            Default is 1000.
        num_points: int
            The number of points to use in your evenly spaced var1_range and var2_range arrays,
        ...
        mc_num_generate: int
            ...
            The number of multivariate random variables to generate when evaluating monte-carlo functions.
            Default is 10,000.
        ranges_to_u: bool
            True to convert user provided var1_range and var2_range arrays to marginal distribution cdf / pseudo-observation values.
            Default is False.
        """
    ...
    def pdf_plot(self, copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], ppf_approx: bool = True, var1_range: np.ndarray = None, var2_range: np.ndarray = None,
        ...
        """Produces a 3D plot of the joint distribution's pdf / density function.
    ...
    def cdf_plot(self, copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], ppf_approx: bool = True, var1_range: np.ndarray = None, var2_range: np.ndarray = None, color: str = 'royalblue', alpha: float = 1.0,
                 ...
                 num_generate: int = 1000, num_points: int = 100, show_progress: bool = True, show: bool = True) -> None:
        """Produces a 3D plot of the joint distribution's cdf / cumulative density function.

        Note that these plots are only implemented when we have 2-dimensional / bivariate distributions.

        This may take time to evaluate for certain copula distributions, due to d-dimensional numerical integration.
        In these case, mc_cdf_plot will likely evaluate faster.
    ...
    def mc_cdf_plot(self, copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], ppf_approx: bool = True, var1_range: np.ndarray = None, var2_range: np.ndarray = None, mc_num_generate: int = 10 ** 4, color: str = 'royalblue', alpha: float = 1.0,
                 figsize: tuple = (8, 8), grid: bool = True, axes_names: tuple = None, zlim: tuple = (None, None),
                 num_generate: int = 1000, num_points: int = 100, show_progress: bool = True, show: bool = True) -> None:
        """Produces a 3D plot of the joint distribution's cdf / cumulative density function, using a monte-carlo numerical approximation.

        Note that these plots are only implemented when we have 2-dimensional / bivariate distributions.

        The standard cdf function may take time to evaluate for certain copula distributions, due to d-dimensional numerical integration.
        In these cases, mc_cdf_plot will likely evaluate faster
        ...
        var2_range: np.ndarray
            ...
            size num_generate and then taking the observed min and max values of each variable from this sample.
        mc_num_generate: int
            The number of multivariate random variables to generate when evaluating mc_cdf.
            Default is 10,000.
        color : str
            The matplotlib.pyplot color to use in your plot.
            Default is 'royalblue'.
    ...
    def copula_pdf_plot(self, copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], var1_range: np.ndarray = None, var2_range: np.ndarray = None, color: str = 'royalblue', alpha: float = 1.0,
                 figsize: tuple = (8, 8), grid: bool = True, axes_names: tuple = None, zlim: tuple = (None, None),
                 num_generate: int = 1000, num_points: int = 100, show_progress: bool = True, show: bool = True) -> None:
        """Produces a 3D plot of the copula distribution's pdf / density function.
        ...
        show: bool
            ...
            Default is True.
        """
        self._threeD_plot(func_str='copula_pdf', copula_params=copula_params, mdists=mdists, ppf_approx=True, var1_range=var1_range,
                          var2_range=var2_range, color=color, alpha=alpha, figsize=figsize, grid=grid, axes_names=axes_names, zlim=zlim,
                          num_generate=num_generate, num_points=num_points, show_progress=show_progress, show=show)

    def copula_cdf_plot(self, copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], var1_range: np.ndarray = None, var2_range: np.ndarray = None, color: str = 'royalblue', alpha: float = 1.0,
                 figsize: tuple = (8, 8), grid: bool = True, axes_names: tuple = None, zlim: tuple = (None, None),
                 num_generate: int = 1000, num_points: int = 100, show_progress: bool = True, show: bool = True) -> None:
        """Produces a 3D plot of the copula distribution's cdf / density function.

        Note that these plots are only implemented when we have 2-dimensional / bivariate distributions.

        This may take time to evaluate for certain copula distributions, due to d-dimensional numerical integration.
        In these case, copula_mc_cdf_plot will likely evaluate faster.
        ...
        mdists : Union[MarginalFitter, dict]
            ...
            to the indices of the variables.
        var1_range: np.ndarray
            numpy array containing a range of values for the u1 variable to plot across.
            If None passed, then an evenly spaced array of length num_points will be generated,
            whose upper and lower bounds are (0.01, 0.99) for copula function plots.
        var2_range: np.ndarray
            numpy array containing a range of values for the u2 variable to plot across.
            If None passed, then an evenly spaced array of length num_points will be generated,
            whose upper and lower bounds are (0.01, 0.99) for copula function plots.
        color : str
            The matplotlib.pyplot color to use in your plot.
        ...
        show: bool
            ...
            Default is True.
        """
        self._threeD_plot(func_str='copula_cdf', copula_params=copula_params, mdists=mdists, ppf_approx=True, var1_range=var1_range,
                          var2_range=var2_range, color=color, alpha=alpha, figsize=figsize, grid=grid, axes_names=axes_names, zlim=zlim,
                          num_generate=num_generate, num_points=num_points, show_progress=show_progress, show=show)

    def copula_mc_cdf_plot(self, copula_params: Union[Params, tuple], mdists: Union[MarginalFitter, dict], var1_range: np.ndarray = None, var2_range: np.ndarray = None, mc_num_generate: int = 10 ** 4, color: str = 'royalblue',
                        alpha: float = 1.0,
                        figsize: tuple = (8, 8), grid: bool = True, axes_names: tuple = None, zlim: tuple = (None, None),
                        num_generate: int = 1000, num_points: int = 100, show_progress: bool = True, show: bool = True) -> None:
        """Produces a 3D plot of the copula distribution's cdf / density function using monte-carlo numerical methods.
        ...
        var1_range: np.ndarray
            numpy array containing a range of values for the u1 variable to plot across.
            If None passed, then an evenly spaced array of length num_points will be generated,
            whose upper and lower bounds are (0.01, 0.99).
        var2_range: np.ndarray
            numpy array containing a range of values for the u2 variable to plot across.
            If None passed, then an evenly spaced array of length num_points will be generated,
            whose upper and lower bounds are (0.01, 0.99).
        mc_num_generate: int
            The number of multivariate random variables to generate when evaluating copula_mc_cdf.
            Default is 10,000.
        color : str
            The matplotlib.pyplot color to use in your plot.
            Default is 'royalblue'.
        ...
        show: bool
            ...
            Default is True.
        """
        self._threeD_plot(func_str='copula_mc_cdf', copula_params=copula_params, mdists=mdists, ppf_approx=True, var1_range=var1_range,
                          var2_range=var2_range, color=color, alpha=alpha, figsize=figsize, grid=grid, axes_names=axes_names, zlim=zlim,
