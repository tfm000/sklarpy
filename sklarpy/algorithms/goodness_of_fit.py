...
__all__ = ['continuous_goodness_of_fit', 'discrete_goodness_of_fit']


def continuous_goodness_of_fit(data: np.ndarray, params: tuple, cdf: Callable, name: str = 'gof') -> pd.DataFrame:
    """
    ...
    cdf : Callable
        The cdf function of the continuous distribution. Must take a numpy array containing data and the distribution's
    ...
    -------
    gof: pd.DataFrame
        A dataframe containing the goodness of fit tests for the continuous distribution.
    ...
    values = [cvm_res.statistic, cvm_res.pvalue, cvm_significant_10pct, cvm_significant_5pct, cvm_significant_1pct,
              ks_stat, ks_pvalue, ks_significant_10pct, ks_significant_5pct, ks_significant_1pct]
    index = ['Cramér-von Mises statistic', 'Cramér-von Mises p-value',
             'Cramér-von Mises @ 10%', 'Cramér-von Mises @ 5%', 'Cramér-von Mises @ 1%',
             'Kolmogorov-Smirnov statistic', 'Kolmogorov-Smirnov p-value',
             'Kolmogorov-Smirnov @ 10%', 'Kolmogorov-Smirnov @ 5%', 'Kolmogorov-Smirnov @ 1%']
    return pd.DataFrame(values, index=index, columns=[name])


def discrete_goodness_of_fit(data: np.ndarray, params: tuple, support: Callable, pdf: Callable, ppf: Callable,
    ...
    params : tuple
        The parameters used to fit the data.
    support : Callable
        A function which generates/calculates the support of the discrete distribution. Must take the distributions
        parameters in a tuple as an argument.
    pdf: Callable
        The pdf function of the discrete distribution. Must take a numpy array containing data and the distribution's
        parameters in a tuple as arguments.
    ppf: Callable
    ...
    -------
    gof: pd.DataFrame
        A dataframe containing the goodness of fit tests for the discrete distribution.
    ...
    """
    num: int = len(data)
    dof: int = num - len(params) - 1  # the degrees of freedom of the fitted distribution
    ...
    # Creating dataframe containing goodness of fit results
    values = [float(chisq_stat), float(chisq_pvalue), chisq_significant_10pct, chisq_significant_5pct,
              chisq_significant_1pct]
    index = ['chi-square statistic', 'chi-square p-value', 'chi-square @ 10%', 'chi-square @ 5%', 'chi-square @ 1%']
    return pd.DataFrame(values, index=index, columns=[name])
