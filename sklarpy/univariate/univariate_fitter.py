# Contains a class used to determine which univariate distribution best fits a dataset
import numpy as np
...
class UnivariateFitter(Savable):
    ...
    def _fit_single_dist(self, name: str, fit_data: np.ndarray, data_type) -> Union[tuple, None]:
    ...
    def _get_distributions(self, distributions: Iterable, multimodal: bool, numerical: bool) -> set:
    ...
    def _fit_processpoolexecutor(self, distributions: set, func: Callable, timeout):
    ...
    def fit(self, distributions: str_or_iterable = None, data_type=None, multimodal: bool = False,
        ...
        distributions: Union[Iterable, str, None]
            ...
            If an iterable, it must contain the string names of the distributions the user wants to fit and/or the
            distribution categories to use.
            If a string, it must be a valid category of distributions or name of a distribution to fit.
            ...
            Note, if a multimodal or numerical category/distribution is specified, the multimodal/numerical arguments
            must also be
            set to True.
            Default is None.
        data_type: Union[str, None]
            The data-type of the distributions to fit if the distributions argument is not given.
            float for continuous distributions and int for discrete distributions. Default is None.
        multimodal: bool
            Whether to include multimodal distributions when fitting if the distributions argument is not given.
            Default is False. Must be True if a multimodal distribution or category is specified in the distributions
            argument.
        numerical: bool
            Whether to include numerical distributions when fitting if the distributions argument is not given.
            Default is False. Must be True if a numerical distribution or category is specified in the distributions
            argument.
        timeout: int
        ...
        for bool_arg in (multimodal, numerical, raise_error, use_processpoolexecutor):
            if not isinstance(bool_arg, bool):
                raise TypeError("multimodal, numerical, raise_error, use_processpoolexecutor arguments must be bool.")
        ...
        distributions: set = self._get_distributions(distributions, multimodal, numerical)

        # fitting distributions
        func: Callable = partial(self._fit_single_dist, fit_data=fit_data, data_type=data_type)
        if use_processpoolexecutor:
            ppe_distributions: set = distributions.intersection(distributions_map['all parametric'])
    ...
    def _filter(self, pvalue: float) -> tuple:
        """method to filter out all distributions which do not statistically fit the data for a given p-value."""
        if not self._fitted:
            raise FitError("UnivariateFitter has not been fitted to data. Called .fit method.")
        ...
        for name in self._fitted_dists:
            ...
            if dist.continuous_or_discrete == 'continuous':
                # Cramér-von Mises and Kolmogorov-Smirnov gof tests for continuous distributions
                cvm_pvalue: float = gof.loc['Cramér-von Mises p-value'].values[0]
                ks_pvalue: float = gof.loc['Kolmogorov-Smirnov p-value'].values[0]

                # keeping only the distributions which are significant for BOTH tests
    ...
    def get_summary(self, sortby: str = None, significant: bool = False, pvalue: float = 0.05) -> pd.DataFrame:
        ...
        sortby: str
            ...
            Default is None.
        significant: bool
            True to remove distributions which fail goodness of fit significance tests.
            Default is False.
        pvalue: float
            The p-value to use when rejecting the null hypothesis in goodness of fit tests.
            Default is 0.05.
        ...
        summary: pd.DataFrame = pd.concat(summaries.values(), axis=1).transpose()
        ...
        if max_num_params != 0:
            ...
            non_param_cols: list = [col for col in cols if col not in param_cols]
            new_order: list = [*cols[:param0_index], *param_cols, *non_param_cols[param0_index:]]
    ...
    def get_best(self, significant: bool = True, pvalue: float = 0.05, raise_error: bool = False, **kwargs):
        ...
        significant: bool
            True to select only from distributions which satisfy goodness of fit significance tests.
            Default is True.
        pvalue: float
            The p-value to use when rejecting the null hypothesis in goodness of fit tests.
            Default is 0.05.
        raise_error: bool
            ...
            an empirical distribution is fitted and returned.
            Default is False.
        ...
        summary: pd.DataFrame = self.get_summary('Sum of Squared Error', significant, pvalue)
        if len(summary) == 0:
    ...
    def plot(self, which: str_or_iterable = 'all', pvalue: float = 0.05, xrange: np.ndarray = None,
             include_empirical: bool = True, empirical_color: str = 'black', qqplot_yx_color: str = 'black',
             alpha: float = 0.5, empirical_alpha: float = 1.0, qqplot_yx_alpha: float = 1.0,
             pdf_plot_limit: float = 1.0, figsize: tuple = (16, 8), grid: bool = True, num_to_plot: int = 100,
             show: bool = True):
        ...
        which: Union[str, Iterable]
            'all', 'best', 'significant', 'best significant' to plot all, the best and significant fitted
            distributions. which can also be the string name of a fitted distribution.
            If an iterable, the distribution names which are specified in the iterable and have been fitted are plotted.
            Default is 'all'.
        pvalue: float
            The p-value to use when rejecting the null hypothesis in goodness of fit tests. Used if which is
            'significant' or 'best significant'.
            Default is 0.05.
        xrange: np.ndarray
        ...
        pdf_plot_limit: float
            The upper limit for pdf values to use in plots. This may be useful as fitted distributions may sometimes
            have unreasonably large pdf values which skew the proportions of the plots, making analysis difficult.
            Default is 1.0.
        alpha: float
        ...
        if isinstance(which, str):
            ...
            if which == 'all':
                ...
                summary: pd.DataFrame = self.get_summary(significant=True, pvalue=pvalue)
                if len(summary) == 0:
                    raise SignificanceError("No statistically significant distributions fitted.")
                dists = summary.index
            elif which == 'best':
                ...
                raise ValueError(f"if which is a string, it must be 'all', 'significant', 'best', 'best significant'"
                                 f" or the name of a fitted distribution, not '{which}'")
        ...
        if xrange is None:
            ...
            xrange: np.ndarray = np.linspace(self._data.min(), self._data.max(), num_to_plot)
        ...
        for str_arg in (empirical_color, qqplot_yx_color):
            if not isinstance(str_arg, str):
                raise TypeError("invalid argument type in plot. check empirical_color, qqplot_yx_color are all strings.")

        for float_arg in (alpha, empirical_alpha, qqplot_yx_alpha, pdf_plot_limit):
            if not isinstance(float_arg, float):
                raise TypeError("invalid argument type in plot. check alpha, empirical_alpha, qqplot_yx_alpha, pdf_plot_limit are all floats.")
        ...
        if include_empirical:
            ...
            ax[0].plot(xrange, empirical_pdf_values, color=empirical_color, alpha=empirical_alpha, label=empirical_label)
            ax[1].plot(xrange, empirical_dist.cdf(xrange), color=empirical_color, alpha=empirical_alpha,
        ...
        for name in dists:
            ...
            ax[3].scatter(ppf_values, empirical_ppf_values, label='Quartiles', alpha=alpha)
